...
export class Year4Component implements OnInit {
  ...
  uploadPastpapers() {
    ...
    this.http.post<{ message: string }>('http://localhost:3000/api/year4pps', paperdata).subscribe(
      (responseData) => {
  ...
  getPapersFromServer() {
    this.http
      .get<{ message: string, papers: any }>(
        'http://localhost:3000/api/year4pps'
      )
      .pipe(map((postData) => {
  ...
  deleteRecord(id: string, i: number) {
    this.http.delete<{ message: string }>('http://localhost:3000/api/year4pps/' + id).subscribe(
      (responseData) => {
