...
@Component({
  selector: 'app-year4',
  templateUrl: './year4.component.html',
  styleUrls: ['./year4.component.css', '../past-papers/past-papers.component.css']
})
export class Year4Component implements OnInit {
  ...
  uploadPastpapers() {
    ...
    this.http.post<{ message: string }>('https://pastpaper-platform.herokuapp.com/api/year4pps', paperdata).subscribe(
      (responseData) => {
  ...
  getPapersFromServer() {
    this.http
      .get<{ message: string, papers: any }>(
        'https://pastpaper-platform.herokuapp.com/api/year4pps'
      )
      .pipe(map((postData) => {
  ...
  deleteRecord(id: string, i: number) {
    this.http.delete<{ message: string }>('https://pastpaper-platform.herokuapp.com/api/year4pps/' + id).subscribe(
      (responseData) => {
